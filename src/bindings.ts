...
async runBuild(options: BuildOptions) : Promise<null> {
    return await TAURI_INVOKE("run_build", { options });
},
async runProcess(options: RunOptions) : Promise<null> {
    return await TAURI_INVOKE("run_process", { options });
}
}
